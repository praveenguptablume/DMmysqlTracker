...
-- 520
delimiter $$
create trigger activityonworkorder_AFTER_INSERT
	after insert
	on activityonworkorder
	for each row
	BEGIN
	DECLARE v_workorderid BIGINT(20);
	DECLARE v_event BIGINT(20);
	DECLARE v_version BIGINT(20);
	DECLARE e_msg VARCHAR(400);
	DECLARE step VARCHAR(30);
	DECLARE v_receiverid VARCHAR(50);
	...
	IF v_event = WO_CREATED OR
	   ...
	   v_event = WO_UNCOMMIT_EMPTY_RETURN THEN
		...
		INSERT INTO X_WORKORDER(WORKORDERID,
								WORKORDERNUMBER,
								ORIGINATORCODE,
								RECEIVERCODE,
								WORKORDERDATE,
								...
								CATEGORY,
								STATUSID,
								DATECREATED, DATERECEIVED, DATEAMENDED, DATEACCEPTED, DATEASSIGNED, DATECOMPLETED,
								DATECANCELED, DATEACTIVE,
								BILLTO,
								VENDORNUMBER,
								REFERENCEID,
								REFERENCEVALUE,
		...
		SET step = 'X_STOP';
		INSERT INTO X_STOP(STOPID, WORKORDERID, STOPNUMBER, STOPTYPE, FACILITYCODE, ADDRESS1, ADDRESS2, CITY, STATE,
						   POSTALCODE, COUNTRY, TELEPHONE, FAX, COMMENTS, REFERENCENUMBER, TIMEZONE, NAME, VERSION,
						   LASTMODIFIEDBY, LASTMODIFIED, CONTACTNAME, CONTACTPHONE)
		SELECT STOPID,
			WORKORDERID,
			STOPNUMBER,
			STOPTYPE,
		...
		INSERT INTO X_EQUIPMENTONWORKORDER (EQUIPMENTONWORKORDERID, WORKORDERID, SHIPPINGID, EQUIPMENTTYPECODE,
											EQUIPMENTID, NETWEIGHT, TAREWEIGHT, GROSSWEIGHT, WEIGHTUNITS,
											SHIPMENTNUMBER, FREIGHTDESCRIPTION,
											SEAL, EMPTYRELEASENUMBER, HAZMAT, OVERWEIGHT, STATUSID, VOLUME, PIECECOUNT,
											COMMENTS, EQUIPMENTNUMBER,
											WEIGHTUOM, VOLUMEUOM, VERSION, LASTMODIFIEDBY, LASTMODIFIED, HEIGHT, WIDTH,
											LENGTH, DIMENSIONUOM, CHASSISNUMBER, REEFERTEMP, REEFERTEMPUNIT)
		SELECT EQUIPMENTONWORKORDERID,
			WORKORDERID,
			SHIPPINGID,
			EQUIPMENTTYPECODE,
			EQUIPMENTID,
		...
		INSERT INTO X_EQUIPMENTHAZMAT (EQUIPMENTHAZMATID, EQUIPMENTWORKORDERID, HMCOMMODITYDESC,
									   HMSHIPPINGNAMEQUALIFIER, HAZARDOUSCLASSIFICATION, HAZARDOUSCLASSQUALIFIER,
									   UNNAIDCODE, WEIGHTUOM, WEIGHT, HMPACKAGINGGROUPCODE, LIMITEDQTYINDICATOR,
									   NOSNAME, TECHNICALNAME, OUTERPACKAGINGUOM, OUTERPACKAGINGQTY, OUTERPACKAGINGDESC,
									   EMERGENCYCONTACTNAME, EMERGENCYCONTACTTELEPHONE, EMERGENCYCONTACTREFERENCE,
									   REPORTABLEQTYFLAG, FLASHPOINTUOM, FLASHPOINTTEMPERATURE,
									   MARINEPOLLUTANTINDICATOR, HAZMATCOMMENTS, VERSION, LASTMODIFIEDBY, LASTMODIFIED,
									   CREATIONTIME)
		SELECT EH.EQUIPMENTHAZMATID,
			EH.EQUIPMENTWORKORDERID,
			EH.HMCOMMODITYDESC,
			EH.HMSHIPPINGNAMEQUALIFIER,
			EH.HAZARDOUSCLASSIFICATION,
			EH.HAZARDOUSCLASSQUALIFIER,
			EH.UNNAIDCODE,
			EH.WEIGHTUOM,
			EH.WEIGHT,
			EH.HMPACKAGINGGROUPCODE,
			EH.LIMITEDQTYINDICATOR,
			EH.NOSNAME,
			EH.TECHNICALNAME,
			EH.OUTERPACKAGINGUOM,
			EH.OUTERPACKAGINGQTY,
			EH.OUTERPACKAGINGDESC,
			EH.EMERGENCYCONTACTNAME,
			EH.EMERGENCYCONTACTTELEPHONE,
			EH.EMERGENCYCONTACTREFERENCE,
			EH.REPORTABLEQTYFLAG,
			EH.FLASHPOINTUOM,
			EH.FLASHPOINTTEMPERATURE,
			EH.MARINEPOLLUTANTINDICATOR,
			EH.HAZMATCOMMENTS,
			v_version,
			EH.LASTMODIFIEDBY,
			SYSDATE(),
			eh.creationtime
		FROM EQUIPMENTHAZMAT EH
				 JOIN EQUIPMENTONWORKORDER EW
					  ON EH.EQUIPMENTWORKORDERID = EW.EQUIPMENTONWORKORDERID
						  AND EW.WORKORDERID = v_workorderid;
		SET step = 'X_CHARGE';
		INSERT INTO X_CHARGE (CHARGEID, STOPID, EQUIPMENTWORKORDERID, SERVICEID, AMOUNT, FSCPERCENT, FSCAMOUNT,
							  CURRENCY, TRANSACTIONNUMBER, COMMENTS, BILLTO, ISBILLABLE, ISINVOICED, CREATEDBY,
							  CREATIONTIME, LASTMODIFIEDBY, LASTMODIFIED, VERSION)
		SELECT CHARGEID,
			STOPID,
			...
			v_version
		FROM CHARGE
		WHERE EQUIPMENTWORKORDERID IN
			  (SELECT EQUIPMENTONWORKORDERID FROM EQUIPMENTONWORKORDER WHERE WORKORDERID = v_workorderid);
		SET step = 'X_APPOINTMENT';
		INSERT INTO X_APPOINTMENT (APPOINTMENTID, EQUIPMENTONWORKORDERID, STOPID, SCHEDULEDTIMESTART, SCHEDULEDTIMEEND,
								   SCHEDULEDTIMEFORM, ACTUALTIME, ACTION, COMMENTS, VERSION, LASTMODIFIEDBY,
								   LASTMODIFIED,
								   DATEACTUALENTERED, GATEOUTEND, GATEOUTACTUAL, GATEOUTFORM, GATEOUTSTART,
								   GATEINSCHEDULECOMMENTS, GATEOUTSCHEDULECOMMENTS, SCHEDULEDLOCATIONCODE,
								   SCHEDULEDLOCATIONCODEQUALIFIER,
								   GATEINACTUALCOMMENTS, GATEOUTACTUALCOMMENTS, ACTUALLOCATIONCODE,
								   ACTUALLOCATIONCODEQUALIFIER, LATITUDE, LONGITUDE, ASSIGNTODRIVERID,
								   ASSIGNTODRIVERNAME, ASSIGNTODRIVERMOBILEPHONE, EXTERNALID, DRIVERNAME,
								   DRIVERMOBILEPHONE)
		SELECT APPOINTMENTID,
			...
			GATEOUTFORM,
			GATEOUTSTART,
			GATEINSCHEDULECOMMENTS,
			GATEOUTSCHEDULECOMMENTS,
			SCHEDULEDLOCATIONCODE,
			SCHEDULEDLOCATIONCODEQUALIFIER,
			GATEINACTUALCOMMENTS,
			GATEOUTACTUALCOMMENTS,
			ACTUALLOCATIONCODE,
			ACTUALLOCATIONCODEQUALIFIER,
			LATITUDE,
			LONGITUDE,
			ASSIGNTODRIVERID,
			ASSIGNTODRIVERNAME,
			ASSIGNTODRIVERMOBILEPHONE,
			EXTERNALID,
			DRIVERNAME,
			DRIVERMOBILEPHONE
		FROM APPOINTMENT
		WHERE EQUIPMENTONWORKORDERID IN
			  (SELECT EQUIPMENTONWORKORDERID FROM EQUIPMENTONWORKORDER WHERE WORKORDERID = v_workorderid);
		SET step = 'X_EVENTEXCEPTION';
		INSERT INTO X_EVENTEXCEPTION (EVENTEXCEPTIONID, VERSION, APPOINTMENTID, EXCEPTIONTYPE, PIECECOUNT,
									  PIECECOUNTVARIANCE, DAMAGE, COMMENTS, WEIGHT, WEIGHTVARIANCE, LASTMODIFIED,
									  REPORTEDDATE,
									  REPORTEDBY)
		SELECT EVENTEXCEPTIONID,
			v_version,
			APPOINTMENTID,
			EXCEPTIONTYPE,
			PIECECOUNT,
			...
			REPORTEDBY
		FROM EVENTEXCEPTION
		WHERE APPOINTMENTID IN (SELECT APPOINTMENTID
								FROM APPOINTMENT
								WHERE EQUIPMENTONWORKORDERID IN (SELECT EQUIPMENTONWORKORDERID
																 FROM EQUIPMENTONWORKORDER
																 WHERE WORKORDERID = v_workorderid));
		SET step = 'X_FREIGHTITEMS';
		INSERT INTO X_FREIGHTITEMS (FREIGHTLINEITEMSID,
									EQUIPMENTWORKORDERID,
									LINEITEMNUMBER,
									UNITID,
									TYPECODE,
	...
	IF v_event = WO_ASSIGNED THEN
		SELECT receivercode INTO v_receiverid FROM WORKORDER WHERE WORKORDERID = v_workorderid;
		UPDATE X_WORKORDER
		SET RECEIVERCODE = v_receiverid
		WHERE workorderid = v_workorderid;
	END IF;
...
CREATE OR REPLACE VIEW `VWEQUIPMENTADVANCEDFILTER` (`APPOINTMENTID`, `WORKORDERID`, `EQUIPMENTONWORKORDERID`, `WORKORDERNUMBER`, `ORIGINATORCODE`, `RECEIVERCODE`, `ORIGINATORNAME`, `RECEIVERNAME`, `CATEGORY`, `CATEGORYTYPE`, `STATUS`, `WORKORDERDATE`, `VESSEL`, `VOYAGE`, `CUTTOFFDATE`, `PORTOFLOADING`, `PORTOFDISCHARGE`, `LASTFREEDAY`, `RESPONDBYDATE`, `DATECREATED`, `DATEACCEPTED`, `DATEASSIGNED`, `DATECOMPLETED`, `DATECANCELED`, `DATEACTIVE`, `SHIPMENTREFERENCENUMBER`, `ORIGINATORIMPORTREFNUMBER`, `EQUIPMENTNUMBER`, `EQUIPMENTTYPECODE`, `ASSOCIATEDUNITID`, `PIECECOUNT`, `STOPNUMBER`, `STOPNAME`, `GISCHEDULETIMEFROM`, `GISCHEDULETIMESTART`, `GISCHEDULETIMEEND`, `GOSCHEDULETIMEFROM`, `GOSCHEDULETIMESTART`, `GOSCHEDULETIMEEND`, `GIACTUALTIME`, `GOACTUALTIME`, `BILLOFLADING`, `BOOKINGNUMBER`, `STOPTYPE`, `CATEGORYID`, `ISINVOICED`, `SCHEDULEDLOCATIONCODE`, `SCHEDULEDLOCATIONCODEQUALIFIER`, `ACTUALLOCATIONCODE`, `ACTUALLOCATIONCODEQUALIFIER`, `GATEINSCHEDULECOMMENTS`, `GATEINACTUALCOMMENTS`, `GATEOUTSCHEDULECOMMENTS`, `GATEOUTACTUALCOMMENTS`, `SHIPMENTNUMBER`, `SEAL`, `EVENTEXCEPTIONID`, `EXCEPTIONFLAG`, `REPORTEDPIECECOUNT`, `DAMAGE`, `EVENTEXCEPTIONCOMMENTS`, `EXCEPTIONREASON`) AS 
  ...
  WO.ORIGINATORCODE ORIGINATORCODE,
  WO.RECEIVERCODE RECEIVERCODE,
...
-- 610
CREATE OR REPLACE VIEW `VW_WO_SHIPMENTEVENTS` (`TABLENAME`, `PRIMARYKEY`, `WORKORDERID`, `WORKORDERNUMBER`, `EVENTCODE`, `EVENTNAME`, `EVENTTIME`, `PUBLISHERCODE`, `ORIGINATORCODE`, `REPORTSOURCE`, `REPORTEDBY`, `CREATEDATE`, `EVENTCOMMENTS`, `EXCEPTIONREASON`) AS 
    ...
    se.eventtime as eventtime, se.publishercode as publishercode,
    se.originatorcode as originatorcode,
    ...
    and se.originatorcode = w.originatorcode
    and (se.publishercode = w.receivercode or se.publishercode=w.originatorcode)
    union
    ...
    ap.actualtime, w.RECEIVERCODE,
    w.originatorcode, ap.lastmodifiedby,
...
CREATE OR REPLACE VIEW `VW_SEARCH_SHIPMENTEVENTS` (`SHIPMENTEVENTID`, `PUBLISHERCODE`, `ORIGINATORCODE`, `ORIGINATORPARENTCODE`, `ORIGINATORNAME`, `EVENTCODE`, `EVENTNAME`, `HOUSEBILL`, `MASTERBILL`, `WORKORDERNUMBER`, `UNITID`, `LONGITUDE`, `LATITUDE`, `LOCATION`, `EVENTTIME`, `REPORTEDBY`, `REPORTSOURCE`, `NOTES`, `CREATEDBY`, `CREATEDATE`, `PUBLISHERID`, `PUBLISHERPARENTCODE`, `ONHAND`, `EQUIPMENTTYPECODE`, `STATUS`, `WORKORDERID`, `RECEIVERNAME`, `EVENTCOMMENTS`, `EXCEPTIONREASON`) AS 
  (
SELECT s.`SHIPMENTEVENTID`,s.`PUBLISHERCODE`,s`ORIGINATORCODE`,s.`ORIGINATORPARENTCODE`,s.`ORIGINATORNAME`,s.`EVENTCODE`,s.`EVENTNAME`,s.`HOUSEBILL`,s.`MASTERBILL`,s.`WORKORDERNUMBER`,s.`UNITID`,s.`LONGITUDE`,s.`LATITUDE`,s.`LOCATION`,s.`EVENTTIME`,s.`REPORTEDBY`,s.`REPORTSOURCE`,s.`NOTES`,s.`CREATEDBY`,s.`CREATEDATE`,s.`PUBLISHERID`,s.`PUBLISHERPARENTCODE`,s.`ONHAND`,s.`EQUIPMENTTYPECODE`, st.description as status, s.`WORKORDERID`, w.receivername, s.`EVENTCOMMENTS`, s.`EXCEPTIONREASON`
FROM shipmentevents s
LEFT OUTER JOIN workorder w
ON s.workordernumber = w.workordernumber and s.originatorcode = w.originatorcode and s.workorderid = w.workorderid
...
alter table notificationtosend
	add ORIGINATOROFFICECODE VARCHAR(50) null;
alter table notificationtosend
	add MOTORCARRIEROFFICECODE VARCHAR(50) null;
